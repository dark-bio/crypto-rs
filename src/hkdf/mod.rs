...
//!
//! ```
//! use darkbio_crypto::hkdf;
//!
//! let secret = [42u8; 32];
//! let encryption = hkdf::key::<32>(&secret, b"", b"example encryption key");
//! let authentication = hkdf::key::<32>(&secret, b"", b"example authentication key");
//! assert_ne!(*encryption, *authentication);
//! ```
...
/// 255 * 32 = 8160 bytes.
pub fn key<const N: usize>(secret: &[u8], salt: &[u8], info: &[u8]) -> Zeroizing<[u8; N]> {
    let salt = if salt.is_empty() { None } else { Some(salt) };
    let (mut prk, hkdf) = Hkdf::<Sha256>::extract(salt, secret);
    prk.as_mut_slice().zeroize();

    let mut output = Zeroizing::new([0u8; N]);
    hkdf.expand(info, output.as_mut()).unwrap();
    output
}

/// key_with_len derives a key from the secret, salt, and info using HKDF-SHA256,
/// returning a byte vector that can be used as a cryptographic key.
///
/// # Panics
///
/// Panics if `out` exceeds the maximum output length for SHA-256 HKDF, which is
/// 255 * 32 = 8160 bytes.
///
...
/// Rust code.
pub fn key_with_len(secret: &[u8], salt: &[u8], info: &[u8], out: usize) -> Zeroizing<Vec<u8>> {
    let salt = if salt.is_empty() { None } else { Some(salt) };
    let (mut prk, hkdf) = Hkdf::<Sha256>::extract(salt, secret);
    prk.as_mut_slice().zeroize();
...
pub fn extract(secret: &[u8], salt: &[u8]) -> Zeroizing<[u8; 32]> {
    let salt = if salt.is_empty() { None } else { Some(salt) };
    let (mut prk, _hkdf) = Hkdf::<Sha256>::extract(salt, secret);
...
///
/// The pseudorandom key should have been generated by [`extract`], or be a
/// uniformly random or pseudorandom cryptographically strong key. See RFC 5869,
/// Section 3.3. Most common scenarios will want to use [`key`] instead.
///
/// # Panics
///
/// Panics if `N` exceeds the maximum output length for SHA-256 HKDF,
/// 255 * 32 = 8160 bytes.
pub fn expand<const N: usize>(prk: &[u8; 32], info: &[u8]) -> Zeroizing<[u8; N]> {
...
/// expand_with_len derives a key, using the given pseudorandom key and optional
/// context info, skipping the extraction step.
///
/// The pseudorandom key should have been generated by [`extract`], or be a
/// uniformly random or pseudorandom cryptographically strong key. See RFC 5869,
/// Section 3.3. Most common scenarios will want to use [`key`] instead.
...
/// # Panics
///
/// Panics if `out` exceeds the maximum output length for SHA-256 HKDF,
/// 255 * 32 = 8160 bytes.
pub fn expand_with_len(prk: &[u8; 32], info: &[u8], out: usize) -> Zeroizing<Vec<u8>> {
    let hkdf = Hkdf::<Sha256>::from_prk(prk).unwrap();
...
mod tests {
    ...
    fn test_hkdf() {
        ...
        fn run<const N: usize>(secret: &str, salt: &str, info: &str, out: &str) {
            ...
            let got = key::<N>(&secret, &salt, &info);
